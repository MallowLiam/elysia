...
class TestAgentCLI:
    """Test cases for AgentCLI."""

    def setup_method(self):
        from elysia.cli import AgentCLI

        self.cli = AgentCLI()

    def test_cli_initialization(self):
        """Test CLI initializes with all components."""
        assert self.cli.agent_manager is not None
        assert self.cli.template_manager is not None
        assert self.cli.agent_executor is not None
        assert self.cli.monitoring is not None

    def test_confirm_action_yes(self):
        """Test confirmation with yes response."""
        with patch("builtins.input", return_value="y"):
            result = self.cli.confirm_action("Test message")
    ...
    def test_list_agents_with_defaults(self):
        """Test listing agents (CLI creates default agents during initialization)."""
        with patch("builtins.print") as mock_print:
            self.cli.list_agents(Mock())
            # CLI creates default agents during initialization, so we expect at least one agent
            assert mock_print.called
            # Check that it doesn't print "No agents found"
            calls = [call.args[0] for call in mock_print.call_args_list]
            assert "No agents found." not in calls

    def test_list_templates(self):
        """Test listing templates."""
        with patch("builtins.print") as mock_print:
            self.cli.list_templates(Mock())
            # Should print templates (may be empty or have defaults)
            assert mock_print.called


class TestTemplateManager:  # pyright: ignore[reportRedeclaration]
    """Test cases for AgentCLI."""
...
def run_tests():
    ...
    # Test AgentCLI
    print("\n5. Testing AgentCLI...")
    cli_test = TestAgentCLI()
    try:
        cli_test.setup_method()
        cli_test.test_cli_initialization()
        cli_test.test_confirm_action_yes()
        cli_test.test_confirm_action_no()
        cli_test.test_list_agents_with_defaults()
        cli_test.test_list_templates()
        print("   ✅ AgentCLI tests passed")
    except Exception as e:
        print(f"   ❌ AgentCLI test failed: {e}")
